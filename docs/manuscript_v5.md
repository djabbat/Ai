...
**Third control (H2, per review M9):** 14 meaningless glyphs in Georgian-style graphics (ჱ̆, ჹ̈, ჿ̆, and 11 more — non-existent signs) with the same 4AFC format. Prediction ladder: აი accuracy > TP ≥ glyphs = chance. This isolates NSM transparency from script-familiarity effects.

**Blinding and independence (per review M8):** stimuli and instructions prepared by an independent assistant blind to hypotheses; participant IDs masked before decision rules are applied; analysis conducted blind (group codes revealed only after PASS/FAIL decisions). The author of the language does not run the sessions.

**Stimulus rendering (per review M10):** all Part B stimuli rendered as SVG/PNG images with a fixed font stack (Noto Sans Georgian + Noto Sans IPA); pre-screening on 5 device classes (Android, iOS, Windows, macOS, Linux); participants with rendering failures excluded and reported.

**Power (H2, per review):** G*Power: one-sample binomial, p0=.25, target accuracy .45, α=.05, power .80 → n ≥ 68 per language group is not required; n=140 trials/language (10 participants × 14 items) yields power > .90.
